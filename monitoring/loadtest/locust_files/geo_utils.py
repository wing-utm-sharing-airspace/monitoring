...
def create_random_flight_path_volume(
    ...
    altitude_lower: int = random.randint(0, 10000),
    altitude_upper: int = 10000,
    start_time: datetime.datetime = datetime.datetime.now(),
    end_time: datetime.datetime = datetime.datetime.now()
    + datetime.timedelta(seconds=10),
):

  rects = create_random_flight_path(
      lat, lng, radius, max_flight_distance_meters
  )
  return rects


def merge_flight_paths(
    flight_paths: list[shapely.Polygon],
) -> shapely.Polygon:
  merged_poly = shapely.unary_union(flight_paths)
  return merged_poly.convex_hull
